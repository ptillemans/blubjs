...
test('the target temperature can be given for monday noon', function (t) {
    ...
    let d = new Date(2018, 5, 25, 12, 0); // monday noon
...
test('when the target temp changes, a SET_TARGET_ACTION is generated', function (t) {
    t.plan(2);

    let state = NEW_SCHEDULE;

    let d1 = new Date(2018, 5, 24, 5, 59); // sunday after wakeup
    t.assert(!scheduler.hasTargetChanged(state, d1));
    let d2 = new Date(2018, 5, 24, 6, 0); // sunday after wakeup
    t.equal(scheduler.hasTargetChanged(state, d2), 20, 'expected new temperature after change');
