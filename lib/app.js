...
function addRoutes(app, store) {
    ...
    app.post('/holiday', jsonParser, function(req, res) {
      var data = req.body;
      console.log('holiday:' + JSON.stringify(data));
      var isHoliday = Number(data.isHoliday);
      if (isHoliday === 0 || isHoliday === 1) {
        store.dispatch(PIDController.createUpdateHoliday(isHoliday));
        ...
        res.status(400).send('isHoliday should be 0 or 1');
      }
    });
