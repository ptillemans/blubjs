...
function startScheduler(store, samplingFunction) {
    ...
    this.job = new CronJob('*/30 * * * * *', function() {
        const changedTarget = hasTargetChanged(store.getState()['schedule']);
        if (changedTarget)
            store.dispatch(PIDController.createSetTargetAction(changedTarget))
    });
