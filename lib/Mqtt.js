...
function connect(store, hostname) {
    let mqttUrl = `mqtt://${hostname}`
    console.log(`subcribing to Mqtt server ${mqttUrl}.`)

    client = mqtt.connect(mqttUrl, {clientId:"blubjs", clean: true});
    ...
    client.on('connect', function() {
        ...
        for (var topic of ['blub/sensors', 'zigbee2mqtt/Buro Lotte Temperature', 'zigbee2mqtt/Mezzanine Temperature']) {
    ...
    client.on('message', function(topic, message) {
        ...
        if (topic === "blub/sensors") {
            ...
            console.log(`process message from sensor ${topic}`)
